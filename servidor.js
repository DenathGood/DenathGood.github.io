...
async function iniciarDB() {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS sesiones (
        id SERIAL PRIMARY KEY, codigo VARCHAR(20) UNIQUE NOT NULL,
        user_id INTEGER REFERENCES usuarios(id), roblox_uid VARCHAR(50),
        job_id VARCHAR(100), nombre VARCHAR(50), activo BOOLEAN DEFAULT TRUE,
        created_at TIMESTAMP DEFAULT NOW()
    )`)
    await pool.query(`CREATE TABLE IF NOT EXISTS comandos (
        id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES usuarios(id),
        roblox_uid VARCHAR(50), job_id VARCHAR(100), comando VARCHAR(50) NOT NULL,
        ejecutado BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT NOW()
    )`)
...
app.post("/registrar", async (req, res) => {
    ...
    try {
        ...
        console.log(`[Roblox] ${nombre} — código: ${codigo}`)
        res.json({ ok: true })
    } catch (e) {
...
app.post("/setcomando", verificarToken, async (req, res) => {
    ...
    try {
        ...
        console.log(`[Cmd] ${s.rows[0].nombre}: "${cmd}"`)
        res.json({ ok: true })
    } catch (e) {
